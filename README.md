# Projeto de TQS

**Contexto do projeto:**
  O produto é uma aplicação online chamada “MagicLook” que serve para alugar roupas de cerimónia, permitindo que utilizadores encontrem, reservem e devolvam roupas para eventos únicos, promovendo a reutilização e reduzindo o desperdício.

**Objetivo:**
Facilitar o acesso a roupas de cerimónia para uso ocasional, garantindo processos simples, sustentáveis e eficientes tanto para clientes como fornecedores.


| Nome | Nmec | Papel |
| -- | -- | -- |
| Maria-Aleksandra Korjenevskaya | 118769 | Product Owner | 
| Gonçalo Floro Garcia Fereira | 120189 | QA Engineer |
| Ivan Horoshko | 120603 | Team Leader and DevOps Engineer |


**JIRA:** https://tqs-project-118769-120189-120603.atlassian.net/jira/software/projects/SCRUM/summary

**SonarCloud:** https://sonarcloud.io/project/overview?id=MagicLook_tqs-project-118769-120151-120603

**PostMan:** https://app.getpostman.com/join-team?invite_code=41cdb4633832e0fee57457ad22ebf7632001677120a7dfaafe6b86268efbf1ed&target_code=25b79d2db5213735a5ea84efc0d36867
